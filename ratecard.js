...
async function setAuditDataInFields(objectId) {
  try {
      ...
      // Check if the status is "Completed" (assuming it's a string)
      if (status === "Completed") {
          ...
          // Use jQuery to hide/show elements
          $("#CreateAuditButtonBlock").hide();
          $("#InvoiceToAuditWrapper").show();
          
          // Call the function to display service data
...
//         }else {
//           // document.getElementById('InvoiceToAuditWrapper').style.display = "none";
...
async function renderInvoiceTable(page) {
  try {
    ...
    // Check the status of the first invoice in the data (assuming there's at least one)
    if (data[0].Audit_Status === "Completed") {
      // Call setAuditDataInFields with the objectId of the first invoice
      setAuditDataInFields(data[0].objectId);
    } else {
      // Display a message here if the first invoice is not "Completed"
      // You can customize this part as needed
      console.log('The first invoice is not "Completed." Display your message here.');
    }
