...
function handleInvoiceData(objectId) {
    ...
    async function fetchAuditData() {
        try { 
            const response = await fetch(`https://cleanstation.backendless.app/api/services/Audit/InvoiceToAudit?Id=${objctID}`);
            if (!response.ok) {
                throw new Error('Network response was not ok.');
            }
            const data = await response.json();
        ...
        tableBody.innerHTML = paginatedDataaudit.map(audit => {
            return `
                <tr>
                    ...
                    <td>
                        <span class="edit-btn glyphicon glyphicon-pencil" onclick="handleauditEdit('${audit.objectId}')"></span>
...
`;
    ...
    // Function to handle Edit button click
    function handleAuditEdit(objectId) {
        alert(`Edit data with objectId: ${objectId}`);
    }
