    ...
    try {
        ...
        if (auditData && auditData.Audit_Status == "Completed") {  
          // Set data in the fields
            ...
            document.getElementById('Additional_Fee.Non-Compliance').innerHTML = auditData.Audit_dashboard.Additional_Fee.Non-Compliance|| 0; 
            ...
            document.getElementById('Operating_Fee.Non-Compliance').innerHTML = auditData.Audit_dashboard.Operating_Fee.Non-Compliance || 0; 
           ...
           // Fuel Surcharge
            ...
            document.getElementById('Fuel_Surcharge.Non-Compliance').innerHTML = auditData.Audit_dashboard.Fuel_Surcharge.Non-Compliance || 0; 
            ...
            document.getElementById('Delivery_Area_Surcharge.Non-Compliance').innerHTML = auditData.Audit_dashboard.Delivery_Area_Surcharge.Non-Compliance || 0; 
            ...
            document.getElementById('Zone.Non-Compliance').innerHTML = auditData.Audit_dashboard.Zone.Non-Compliance || 0; 
            ...
            document.getElementById('Packaging_Audit.Non-Compliance').innerHTML = auditData.Audit_dashboard.Packaging_Audit.Non-Compliance || 0; 
            ...
            document.getElementById('Weight_Audit.Non-Compliance').innerHTML = auditData.Audit_dashboard.Weight_Audit.Non-Compliance || 0; 
...
function displayServiceData(auditData) {
  ...
  if (totalbaseorder.length === 0) {
      ...
      totalbaseorder.forEach(item => {
          ...
          const percentageOfOrders = ((totalOrders / auditData.Invoice_dashboard.No_of_orders) * 100).toFixed(2);
...
function displayStorageData(auditData) {
 ...
 if (totalStorage.length === 0) {
   ...
   totalStorage.forEach(item => {
     ...
     const totalOrders = item.Quantity || 0;
     totalOrdersCell.textContent = totalOrders;
   });
 }
   ...
   if (Zones.length === 0) {
     ...
     Zones.forEach(item => {
       ...
       totalZoneCell.textContent = item.Orders || 'N/A';
         ...
         // Calculate percentage
     const percentageOfOrders = ((totalZoneCount / auditData.Invoice_dashboard.No_of_orders) * 100).toFixed(3);
     percentageCell.textContent = `${percentageOfOrders}%`;
     });
   }
}
...
function displayStateData(auditData) {
     ...
     if (State.length === 0) {
       ...
       State.forEach(item => {
         ...
         const percentageCell = row.insertCell(2);
         const totalState = item.State || 0;
         StateCell.textContent = totalState;


         const totalStateCount = item.Orders || 0;
         totalStateCell.textContent = item.Orders || 'N/A';


         // Calculate percentage
         const percentageOfOrders = ((totalStateCount / auditData.Invoice_dashboard.No_of_orders) * 100).toFixed(3);
         percentageCell.textContent = `${percentageOfOrders}%`;
       });
     }
  }
...
async function renderInvoiceTable(page) {
  ...
  tableBody.innerHTML = paginatedDatainvoice
    .map((invoice) => {
      ...
      const statusColor = status === "Completed" ? "green" : status === "Ready for Audit" ? "orange" : "red";
