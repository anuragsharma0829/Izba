...
async function setAuditDataInFields(objectId) {
    try {
        ...
        if (auditData) {
          ...
          var invoiceAuditStatus = localStorage.getItem("audit-status");
          if (invoiceAuditStatus) {
            document.getElementById('auditStatusDisplay').innerHTML = invoiceAuditStatus;
        }
    ...
    .map((invoice) => {
      ...
      return `
             <tr>
                 ...
                 <td style="color: ${statusColor}">${status} ${localStorage.setItem("audit-status", status)}</td>
