...
$(document).ready(function () {
  ...
  $("#goToReciving").click(function () {
    ...
    if (
      ...
      errorElement.innerHTML = "Please fill all the required fields.";
...
$(document).ready(function () {
  ...
  $(document).ready(function () {
    ...
    function checkInputs() {
      ...
      // Add similar lines for other input pairs here
      ...
      var StandardHourlyLaborValue = $("#StandardHourlyLaborInput")
        .val()
        .trim();
      $("#WarehouseHoursInput").prop(
        "readonly",
        StandardHourlyLaborValue === ""
      );
    ...
    $("#gotoEstimateDetail").click(function () {
      ...
      let estimatorDetails = {
        ReceiptPallet:
          PalletsReceivedInput.trim() !== ""
            ? parseFloat(PalletsReceivedInput)
            : 0,
        ReceiptCase:
          casesReceived.trim() !== "" ? parseFloat(casesReceived) : 0,
        ...
        .then((data) => {
          ...
          fetch(
            ...
            .then((data) => {
              ...
              fetch(
                `https://cleanstation.backendless.app/api/data/Estimate/${EstimateId}`
              )
                .then((res) => res.json())
                .then((data) => {
                  // pic fee
                  ...
                  let TotalPickCharges =
                    baseCostTotal + totalExtPic + TotalReturns + totalKitCosts;
                  // Total reciving
                  let TotalReciving =
                  ...
                  //  labour
                  let TotalLaburCharge =
                    Labor_Warehouse_Rate * WHHours +
                    Labor_OT_Rate * OTHours +
                    Labor_IT_Rate * ITHours +
                    Management_Fee;
                  ...
                  document.getElementById("TotalSpent").textContent =
                    "$" + TotalSpents;
                  document.getElementById("TotalAveragePerOrder").textContent =
                    "$" + average;
                  ...
                  fetch(
                    ...
                    .then((getTotals) => {
                      document.getElementById("TotalPickFeesCalc").textContent =
                        "$" + getTotals.TotalPickCharge;
                      document.getElementById("TotalStorage").textContent =
                        "$" + getTotals.TotalStorageCharge;
                      document.getElementById("TotalkReceipt").textContent =
                        "$" + getTotals.TotalReceiptCharge;
                      document.getElementById("TotalMisclabor").textContent =
                        "$" + getTotals.TotalMiscCharge;
                      // Now create the chart with the updated data
                      let totalStorage = getTotals.TotalStorageCharge;
                      let totalpicCharges = getTotals.TotalPickCharge;
                      let totalRecipet = getTotals.TotalReceiptCharge;
                      let totalMisc = getTotals.TotalMiscCharge;
  ...
  $(document).ready(function () {
    $("#addFccenter").click(function () {
      ...
      fetch(
        ...
        .then((data) => {
          //  add success here
          // You can add any other actions you want to perform after successful API call here
          // For example, hiding/showing sections or resetting the form.
        })
        .catch((error) => {
          Swal.fire({
            icon: "error",
            title: "Error",
            text: "An error occurred while adding the Fulfillment Center.",
          });
      ...
      .then((data) => {
        ...
        $("#FinalupdateButton").click(function () {
          ...
          const UpdateEstimatorqunt = {
            ...
            // ReturnsCost: TotalReturns,
            // KitCost: totalKitCosts,
            // TotalStorageCharge: TotalStoragee,
            // TotalMiscCharge: TotalLaburCharge
          };
          ...
          fetch(
            ...
            .then((updatedData) => {
              ...
              document.getElementById(
                "startDateText"
              ).innerHTML = `${startDate.getDate()}/${startDate.getMonth() + 1
                }/${startDate.getFullYear()}`;
              document.getElementById(
                "endDateText"
              ).innerHTML = `${endDate.getDate()}/${endDate.getMonth() + 1
                }/${endDate.getFullYear()}`;
          ...
          // Chart donute
          let totalStorage = 233;
          let totalpicCharges = 54;
          let totalRecipet = 67;
          let totalMisc = 98;
...
//   errorElement.style.color = "red";
//   errorElement.style.display = "block";
//   startDate.value = ""; // Reset the start date input value
//   endDate.value = ""; // Reset the end date input value
//   endDate.disabled = true; // Disable the end date input
// } else {
//   errorElement.style.display = "none";
//   endDate.disabled = false; // Enable the end date input
// }

// // Set the minimum selectable date in the end date picker
// const minEndDate = new Date(startDate);
// minEndDate.setDate(minEndDate.getDate() + 1); // Increment by 1 day to disable the selected start date
// endDate.min = minEndDate.toISOString().slice(0, 10);
  ...
  $(document).ready(function () {
    $("#SignUpButton").click(function () {
      ...
      if (firstName === "") {
        ...
        fnameerror.text("First Name should not be blank");
        fnameerror.css({ color: "red", display: "block" });
        return false;
      } else {
        const fnameerror = $("#fnameerror");
        fnameerror.text("");
      ...
      if (lastName === "") {
        ...
        lnameerror.text("Last Name should not be blank");
        lnameerror.css({ color: "red", display: "block" });
        return false;
      } else {
        const lnameerror = $("#lnameerror");
        lnameerror.text("");
      ...
      if (emailAddress === "") {
        ...
        emailerror.text("Email Address should not be blank");
        emailerror.css({ color: "red", display: "block" });
        return false;
      } else {
        const emailerror = $("#emailerror");
        emailerror.text("");
      ...
      if (signUpPassword === "") {
        ...
        password.text("Last Name should not be blank");
        password.css({ color: "red", display: "block" });
        return false;
      } else {
        const password = $("#password");
        password.text("");
      ...
      if (signUpConfirmPassword === "") {
        ...
        confirmPassword.text("Confirm Password should not be blank");
        confirmPassword.css({ color: "red", display: "block" });
        return false;
      } else {
        const confirmPassword = $("#confirmpass");
      ...
      if (signUpPassword !== signUpConfirmPassword) {
        const confirmPassword = $("#confirmpass");
        confirmPassword.text("Password and Confirm Password do not match");
        confirmPassword.css({ color: "red", display: "block" });
        return false;
      } else {
        const confirmPassword = $("#confirmpass");
