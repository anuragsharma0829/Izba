...
$(document).ready(function () {
  ...
  $("#goToEstimates").click(function () {
    ...
    function handleFcobjId(id) {
      ...
      // api to update BrandTable  https://cleanstation.backendless.app/api/data/Brand/3B46814B-604E-4576-87FF-626CE1E48F23/Fulfillment_Contract
      // fetch(`https://cleanstation.backendless.app/api/data/Brand/${postId}`/Fulfillment_Contract, {
      fetch(
    ...
    $("#gotoEstimateDetail").click(function () {
      ...
      fetch("https://cleanstation.backendless.app/api/data/Estimate", {
        ...
        .then((data) => {
          ...
          alert("Quantity Set sucess");
          console.log(data.objectId,'?????????????????????');
        })
        .catch((error) => console.error("Error:", error));


     // Define the variables before the fetch request
let TotalOrders;
let ExtraPicks;
let Returns;
let KitsBuilt;
let PalletsReceivedInput;
let casesReceived;
let palletStorage;
let shelfStorage;
let binStorage;
...
//storage
let Storage_Per_Pallet;
let Storage_Per_Shelf;
let Storage_Per_Bin; 
  
//Reciving
let Receipt_Per_Pallet_Fee;
let Receipt_Per_Case_Fee;
// labour
let Labor_Warehouse_Rate;
let Labor_OT_Rate;
let Labor_IT_Rate;
...
fetch(`https://cleanstation.backendless.app/api/data/Fulfillment_Contract/${fcobjId}`)
  ...
  .then((fulfillmentData) => {
    ...
    // pick fee part
    Base_Per_Order_Fee = fulfillmentData.Base_Per_Order_Fee;
    Additional_Pick_Fee = fulfillmentData.Additional_Pick_Fee;
    Base_Per_Return_Fee = fulfillmentData.BaseReturnFeeInput;
    Additional_Pick_Kit = fulfillmentData.Additional_Pick_Kit;

    // Storage fee part
    Storage_Per_Pallet = fulfillmentData.Storage_Per_Pallet;
    Storage_Per_Shelf = fulfillmentData.Storage_Per_Shelf;
    Storage_Per_Bin = fulfillmentData.Storage_Per_Bin;

    //reciving
    Receipt_Per_Pallet_Fee = fulfillmentData.Receipt_Per_Pallet_Fee;
    Receipt_Per_Case_Fee = fulfillmentData.Receipt_Per_Case_Fee;

    //labour
    Labor_Warehouse_Rate = fulfillmentData.Labor_Warehouse_Rate;
    Labor_OT_Rate = fulfillmentData.Labor_OT_Rate;
    Labor_IT_Rate = fulfillmentData.Labor_IT_Rate;

    // Calculate TotalPickCharge
    const TotalPickCharge =
      Base_Per_Order_Fee * TotalOrders +
      Additional_Pick_Fee * ExtraPicks +
      Base_Per_Return_Fee * Returns +
      Additional_Pick_Kit * KitsBuilt;

    // Calculate TotalReceiptCharge
    const TotalReceiptCharge =
      Receipt_Per_Pallet_Fee * ReceiptPallet +
      Receipt_Per_Case_Fee * ReceiptCase;

    // Calculate TotalStorageCharge
    const TotalStorageCharge =
      Storage_Per_Pallet * Pallets +
      Storage_Per_Shelf * Shelfs +
      Storage_Per_Bin * Bins;

    // Calculate TotalMiscCharge (replace the placeholders with the actual variables)
    const TotalMiscCharge = YOUR_CALCULATION_HERE;

    // Update the UpdateestimatorDetails object with the calculated values
    let UpdateestimatorDetails = {
      ReceiptPallet: parseFloat(PalletsReceivedInput) ?? 0,
      ReceiptCase: parseFloat(casesReceived) ?? 0,
      Pallets: parseFloat(palletStorage) ?? 0,
      Shelfs: parseFloat(shelfStorage) ?? 0,
      ...
      OTHours: parseFloat(otHours) ?? 0,
      ITHours: parseFloat(itHours) ?? 0,
      TotalPickCharge,      // Update with the calculated TotalPickCharge
      TotalReceiptCharge,   // Update with the calculated TotalReceiptCharge
      TotalStorageCharge,   // Update with the calculated TotalStorageCharge
      TotalMiscCharge,      // Update with the calculated TotalMiscCharge
    };

    // Now you can perform the PUT request to update the data
    fetch(
      `https://cleanstation.backendless.app/api/data/Estimate/${EstimatPostid}/Fulfillment_Contract`,
      {
        ...
        body: JSON.stringify(UpdateestimatorDetails),
      }
    )
      .then((response) => response.json())
  ...
  .catch((error) => console.error("Error fetching Fullfillment data:", error));
...
//   });
  ...
  // $(document).ready(function () {
  //   $("#gotoEstimateDetail").click(async function () {
  //     $("#GetStartedSection").hide();
  //     $("#EstimateDetailSection").show();
  //     $("#EstimateEnvoice").show();
  ...
  //     // Call API
  //     try {
  //       const response = await fetch("https://cleanstation.backendless.app/api/data/Estimate", {
  //         method: "POST",
  //         body: JSON.stringify(estimatorDetails),
  //         headers: {
  //           "Content-type": "application/json; charset=UTF-8",
  ...
  //       alert("Quantity Set success");
  //       console.log(data.objectId, "?????????????????????");
  
  //       await new Promise((resolve) => setTimeout(resolve, 2000)); // Wait for 2000ms
  //       calculation(); // Call the 'calculation' function after the wait
  //     } catch (error) {
...
//   // Handle the "Estimator" button click event
//   document.getElementById("gotoEstimateDetail").addEventListener("click", async () => {
//     try {
//       // Brand data
//       const brandResponse = await fetch(`https://cleanstation.backendless.app/api/data/Brand/${postId}`);
//       const brandData = await brandResponse.json();
//       console.log("Brand data:", brandData);

//       // Fullfillment data
//       const fulfillmentResponse = await fetch(`https://cleanstation.backendless.app/api/data/Fulfillment_Contract/${fcobjId}`);
...
//       // Estimator data
//       const estimateResponse = await fetch(`https://cleanstation.backendless.app/api/data/Estimate/${EstimatPostid}?loadRelations=Fulfillment_Contract`);
//       const estimateData = await estimateResponse.json();
//       console.log("Estimate data:", estimateData);
//       TotalOrders = estimateData.TotalOrders;
//       ExtraPicks = estimateData.ExtraPicks;
//       Returns = estimateData.Returns;
//       KitsBuilt = estimateData.KitsBuilt;

//       // Log the variable values for debugging
//       console.log("Base_Per_Order_Fee:", Base_Per_Order_Fee);
//       console.log("TotalOrders:", TotalOrders);
//       console.log("Additional_Pick_Fee:", Additional_Pick_Fee);
//       console.log("ExtraPicks:", ExtraPicks);
//       console.log("Base_Per_Return_Fee:", Base_Per_Return_Fee);
//       console.log("Returns:", Returns);
//       console.log("Additional_Pick_Kit:", Additional_Pick_Kit);
//       console.log("KitsBuilt:", KitsBuilt);

//       // Perform the calculation
//       if (Base_Per_Order_Fee && TotalOrders) {
//         TotalPickFeesCount += Base_Per_Order_Fee * TotalOrders;
//       }
//       if (Additional_Pick_Fee && ExtraPicks) {
//         TotalPickFeesCount += Additional_Pick_Fee * ExtraPicks;
//       }
//       if (Base_Per_Return_Fee && Returns) {
//         TotalPickFeesCount += Base_Per_Return_Fee * Returns;
//       }
//       if (Additional_Pick_Kit && KitsBuilt) {
//         TotalPickFeesCount += Additional_Pick_Kit * KitsBuilt;
//       }

//       // Set the total pick fees count
//       document.getElementById("TotalPickFeesCalc").textContent = TotalPickFeesCount;
//       console.log("TotalPickFeesCount:", TotalPickFeesCount);
//     } catch (error) {
//       console.error("Error:", error);
//     }
//   });
...
//     // Estimator data
//     fetch(`https://cleanstation.backendless.app/api/data/Estimate/${EstimatPostid}?loadRelations=Fulfillment_Contract`)
//       .then((res) => res.json())
//       .then((estimateData) => {
//         console.log("Estimate data:", estimateData);
//         // pick feee part //
...
//          // Storage feee part //
//          Pallets = estimateData.Pallets;
//          Shelfs = estimateData.Shelfs;
//          Bins = estimateData.Bins;

//          //Reciving
//          ReceiptPallet = estimateData.ReceiptPallet;
//          ReceiptCase = estimateData.ReceiptCase;

//          //labour
//          WHHours = estimateData.WHHours;
//          OTHours = estimateData.OTHours;
//          ITHours = estimateData.ITHours;

//         // Perform the calculation
//         TotalPickFeesCount = 0;
//         if (Base_Per_Order_Fee && TotalOrders) {
//           TotalPickFeesCount += Base_Per_Order_Fee * TotalOrders;
//         }
//         if (Additional_Pick_Fee && ExtraPicks) {
...
//         console.log("TotalPickFeesCount:", TotalPickFeesCount);
//       })
//       .catch((error) => console.error("Error fetching Estimator data:", error));
//   }

//   // Handle the "Estimator" button click event
//   document.getElementById("gotoEstimateDetail").addEventListener("click", performCalculation);

//   // Handle the "second_estimate" button click event
//   document.getElementById("FinalupdateButton").addEventListener("click", performCalculation);
// }
