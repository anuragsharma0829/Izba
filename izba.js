...
$("#updateButton").click(function () {
  ...
  fetch(`https://cleanstation.backendless.app/api/data/Fulfillment_Contract/${fcobjId}`)
    ...
    .then((data) => {
      ...
      const startDate = isValidDate(data.Start_Date) ? data.Start_Date : '';
      const endDate = isValidDate(data.End_Date) ? data.End_Date : '';
      document.getElementById('updatestart').value = startDate;
