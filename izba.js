...
// }
function calculateTotalPickFees() {
  let TotalPickFeesCount;
  let Base_Per_Order_Fee;
  let TotalOrders;
  let Additional_Pick_Fee;
...
fetch(`https://cleanstation.backendless.app/api/data/Fulfillment_Contract/${fcobjId}`)
  ...
  .then((data) => {
    ...
    Base_Per_Return_Fee = data.BaseReturnFeeInput;
    Additional_Pick_Kit = data.Additional_Pick_Kit;

    // Check if all the required data is available before performing the calculation
    if (Base_Per_Order_Fee && TotalOrders && Additional_Pick_Fee && ExtraPicks && Base_Per_Return_Fee && Returns && Additional_Pick_Kit && KitsBuilt) {
      calculateTotalPickFees();
    }
  })
  .catch((error) => {
...
// Fetch the remaining data and update the corresponding variables
