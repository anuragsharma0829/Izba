...
$(document).ready(function () {
  ...
  $("#goToReciving").click(function () {
    ...
    if (endDate < startDate) {
      errorElement.innerHTML = "End date cannot be before start date.";
...
$(document).ready(function () {
  ...
  $(document).ready(function () {
    ...
    $("#receiptPerPalletInput, #storagePerPalletInput, #storagePerShelfInput,#receiptPerCaseInput,#storagePerBin,#BaseFeePerOrderInput,#AdditionalPickFeeInput,#BaseReturnFeeInput,#AssemblyPerKitInput,#StandardHourlyLaborInput,#OTHourlyLaborInput,#ITHourlyLaborInput").on("input", function () {
      checkInputs();
    });
    ...
    function checkInputs() {
     ...
     // Define the saveEstimateData function
     async function saveEstimateData() {
      try {
        const apiUrl = 'https://meekprotest.backendless.app/api/services/AdditionService/saveAndFetchEstimate';

        // Create the input data object with all the required properties
        const inputData = {
          ReceiptPallet: 10,
          ReceiptCase: 20,
          Pallets: 5,
          ...
          OTHours: 50,
          ITHours: 100,
          EstimateId: "2B557F30-6D1D-4E11-89F1-513E1BECC33E", // Replace with the actual EstimateId
        };
        ...
        if (response.ok) {
          ...
          // Check if EstimateId exists in responseData
          if ('EstimateId' in responseData) {
            // Use EstimateId if available
            const EstimateId = responseData.EstimateId;
            console.log("EstimateId:", EstimateId);
            localStorage.setItem('EstimateId', EstimateId);
          } else {
            // Use objectId if EstimateId is not available
            const EstimateId = responseData.objectId;
            console.log("EstimateId:", EstimateId);
            localStorage.setItem('EstimateId', EstimateId);
          }

          // Call the function to handle the retrieved EstimateId
          handleEstimateId(localStorage.getItem('EstimateId'));
        } else {
