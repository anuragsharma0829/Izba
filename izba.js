...
function calculation() {
  ...
  promises.push(fulfillmentPromise);

  // Estimator data
  const estimatorPromise = fetch(`https://cleanstation.backendless.app/api/data/Estimate/${EstimatPostid}?loadRelations=Fulfillment_Contract`).then((res) => res.json());
  promises.push(estimatorPromise);
