...
async function calculation() {
  ...
  try {
    ...
    const Bdata = await brandResponse.json();
    console.log("Brand data:", Bdata);

    // Fulfillment data
    const fulfillmentResponse = await fetch(`https://cleanstation.backendless.app/api/data/Fulfillment_Contract/${fcobjId}`);
    const Fdata = await fulfillmentResponse.json();
    console.log("Fulfillment data:", Fdata);
    Base_Per_Order_Fee = Fdata.Base_Per_Order_Fee;
    Additional_Pick_Fee = Fdata.Additional_Pick_Fee;
    Base_Per_Return_Fee = Fdata.BaseReturnFeeInput;
    Additional_Pick_Kit = Fdata.Additional_Pick_Kit;
    ...
    // Estimator data
    const estimatorResponse = await fetch(`https://cleanstation.backendless.app/api/data/Estimate/${EstimatPostid}?loadRelations=Fulfillment_Contract`);
    const Edata = await estimatorResponse.json();
    console.log("Estimator data:", Edata);
    TotalOrders = Edata.TotalOrders;
    ExtraPicks = Edata.ExtraPicks;
    Returns = Edata.Returns;
    KitsBuilt = Edata.KitsBuilt;
