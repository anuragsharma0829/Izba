...
function calculation() {
  ...
  const brandDataPromise = fetch(`https://cleanstation.backendless.app/api/data/Brand/${postId}`).then((res) => res.json());
  const fulfillmentDataPromise = fetch(`https://cleanstation.backendless.app/api/data/Fulfillment_Contract/${fcobjId}`).then((res) => res.json());
  const estimatorDataPromise = fetch(`https://cleanstation.backendless.app/api/data/Estimate/${EstimatPostid}?loadRelations=Fulfillment_Contract`).then((res) => res.json());

  // Wait for all the promises to resolve
  Promise.all([brandDataPromise, fulfillmentDataPromise, estimatorDataPromise])
    .then(([brandData, fulfillmentData, estimatorData]) => {
      ...
      TotalOrders = estimatorData.TotalOrders;
      ExtraPicks = estimatorData.ExtraPicks;
      Returns = estimatorData.Returns;
      KitsBuilt = estimatorData.KitsBuilt;
