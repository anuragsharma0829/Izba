...
function calculation() {
  ...
  const brandPromise = fetch(`https://cleanstation.backendless.app/api/data/Brand/${postId}`).then((res) => res.json());
  const fulfillmentPromise = fetch(`https://cleanstation.backendless.app/api/data/Fulfillment_Contract/${fcobjId}`).then((res) => res.json());
  const estimatePromise = fetch(`https://cleanstation.backendless.app/api/data/Estimate/${EstimatPostid}`).then((res) => res.json());

  // Wait for all the promises to resolve
  Promise.all([brandPromise, fulfillmentPromise, estimatePromise])
    .then(([brandData, fulfillmentData, estimateData]) => {
...
// Event listener for the estimator button click
document.getElementById("gotoEstimateDetail").addEventListener("click", calculation);
