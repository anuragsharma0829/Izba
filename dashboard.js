          ...
          contractsToShow.forEach((contract, index) => {
            ...
            row.innerHTML = `
                ...
                <td style="padding: 8px; border-bottom: 1px solid #ccc;">
                  <a class="contract-name" data-contract-id="${contract.objectId}"> ${contract.Contract_name} </a>
                </td>
                <td style="padding: 8px; border-bottom: 1px solid #ccc;">
      ...
      fileReader.onload = async function (event) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            $("#BinFee").val(
              parsedValues["Storage_Per_Bin"]?.value?.replace(/\$|,/g, "") || 0
            );
            ...
            $("#laborOtrate").val(
              parsedValues["Labor_OT_Rate"]?.value
                ?.replace(/\$|,/g, "")
                .replace("Hourly", "")
            ) || 0;
            $("#laborItrate").val(
              parsedValues["Labor_IT_Rate"]?.value
                ?.replace(/\$|,/g, "")
                .replace("Hourly", "")
            ) || 0;
            ...
            // Managment
            $("#ManagmentFee").val(
              parsedValues["Management_Fee"]?.value?.replace(/\$|,/g, "") || 0
            );
            $("#MinimumFee").val(
              parsedValues["Minimum_Fee"]?.value?.replace(/\$|,/g, "") || 0
            );

            // parcel
            $("#ParcelManifesFee").val(
              parsedValues["Manifest_Fee"]?.value?.replace(/\$|,/g, "") || 0
            );
