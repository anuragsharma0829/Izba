...
// }
    ...
    async function fetchAndShowContracts() {
    ...
    async function renderContractTable(page) {
        const data = await fetchAndShowContracts();
        ...
        tableBody.innerHTML = paginatedDatacontract.map(contract => {
            ...
            return `
                <tr>
                    <td onclick="fetchAndShowContractDetails('${contract.objectId}')">${contract.Contract_name}</td>
...
function nextContractPage() {
    fetchAndShowContracts().then(data => {
