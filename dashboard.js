...
async function populateDropdown() {
    ...
    // Show the delete button after populating the dropdown
...
async function handleProductSelection() {
    ...
    //     const defaultProductId = productData[0].objectId;
    //     dropdown.value = defaultProductId;
    //     // Call the productsAlldata function with the defaultProductId
    //     productsAlldata(defaultProductId);
    // }
...
async function fetchProductData() {
    try {
        // const response = await fetch(`https://cleanstation.backendless.app/api/services/Product/UserProduct?ID=AF00B19D-5B3B-4840-8304-8EF0005DB39B`);
        const response = await fetch(`https://cleanstation.backendless.app/api/services/Product/UserProduct?ID=${objID}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
