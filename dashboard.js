...
$("#mainSection").click(function () {
    $("#Detail, #Create, #Update, #RCDetail, #RCCreate, #RCUpdate, #CreateService").hide();
            ...
            $("#UpdateContractRateButton").click(function () {
                ...
                const requestBody = {
                    ...
                    Fulfillment_Center_ID: {
                        objectId: localStorage.getItem("selectedEDITCenterId") || localStorage.getItem("selectedCenterId")
                    },
      ...
      `;
        ...
        if (
            ...
            item.Zone_Cards.forEach(function (zoneCard) {
                ...
                tableRow.innerHTML = `
            <td  style="background-color: green; color: white; height: 30px; text-align: center;">${zoneCard["Weight_" + item.Weight_Unit]
                    } ${item.Weight_Unit}</td>
            
            
      <td style ="padding: 2px 5px; text-align: center;"><input type="text" class="demo" data-object-id="${item.objectId
            ...
            fileReader.onload = async function (event) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        $("#Baseorderfee").val(extractNumericValue(parsedValues['Base_Per_Order_Fee']?.value?.replace(/\$|,/g, '')));
                        $("#additionalPickfee").val(extractNumericValue(parsedValues['Additional_Pick_Fee']?.value?.replace(/\$|,/g, '')));
                        $("#BaseReturnFee").val(extractNumericValue(parsedValues['Base_Per_Return_Fee']?.value?.replace(/\$|,/g, '').replace('Hourly', '')));
                        $("#PerKitFee").val(extractNumericValue(parsedValues['Additional_Pick_Kit']?.value?.replace(/\$|,/g, '').replace('Hourly', '')));

                        // storage 
                        $("#PalletFee").val(extractNumericValue(parsedValues['Storage_Per_Pallet']?.value?.replace(/\$|,/g, '')) || 0);
                        $("#SelfFee").val(extractNumericValue(parsedValues['Storage_Per_Shelf']?.value?.replace(/\$|,/g, '')) || 0);
                        $("#BinFee").val(extractNumericValue(parsedValues['Storage_Per_Bin']?.value?.replace(/\$|,/g, '')) || 0);

                        // receving
                        $("#palletReciiptRate").val(extractNumericValue(parsedValues['Receipt_Per_Pallet_Fee']?.value?.replace(/\$|,/g, '')) || 0);
                        $("#CaseRecipRate").val(extractNumericValue(parsedValues['Receipt_Per_Case_Fee']?.value?.replace(/\$|,/g, '')) || 0);

                        // labour
                        $("#WHlabor").val(extractNumericValue(parsedValues['Labor_Warehouse_Rate']?.value?.replace(/\$|,/g, '').replace('Hourly', '')) || 0);
                        $("#laborItrate").val(extractNumericValue(parsedValues['Labor_IT_Rate']?.value) || 0);
                        $("#laborOtrate").val(extractNumericValue(parsedValues['Labor_OT_Rate']?.value) || 0);

                        // Managment 
                        $("#ManagmentFee").val(extractNumericValue(parsedValues['Management_Fee']?.value?.replace(/\$|,/g, '')) || 0);
                        $("#MinimumFee").val(extractNumericValue(parsedValues['Minimum_Fee']?.value?.replace(/\$|,/g, '')) || 0);

                        // parcel
                        $("#ParcelManifesFee").val(extractNumericValue(parsedValues['Manifest_Fee']?.value?.replace(/\$|,/g, '')) || 0);
                        $("#internationalFee").val(extractNumericValue(parsedValues['International_Surcharge_Fee']?.value?.replace(/\$|,/g, '')) || 0);
