  ...
  function fetchAndShowContractDetails(contractId) {
    ...
    fetch("https://cleanstation.backendless.app/api/services/FulfilmentContract/IDToContractDetail", {
      ...
      .catch((error) => console.error("Error fetching contract details:", error));
       ...
       // Global variables
      let currentPage = 1;
      const itemsPerPage = 5; // You can change the number of items per page here
          ...
          tableBody.innerHTML = paginatedData.map(item => {
              ...
              return `
                  <tr>
                      ...
                      <td>
                          <span class="edit-btn glyphicon glyphicon-pencil" onclick="handleEdit('${item.objectId}')"></span>
                          <span class="delete-btn glyphicon glyphicon-trash" onclick="handleDelete('${item.objectId}')"></span>
                      </td>
                  </tr>
              `;
          }).join('');
      ...
      function renderPagination(totalItems) {
          ...
          for (let i = 1; i <= totalPages; i++) {
              paginationHtml += `<li${currentPage === i ? ' class="active"' : ''}><a href="#" onclick="changePage(${i})">${i}</a></li>`;
          }
          paginationHtml += `
      ...
      .then((data) => {
        ...
        $("#UpdateContractRateButton").click(function () {
          ...
          fetch("https://cleanstation.backendless.app/api/services/FulfilmentContract/Update", {
            ...
            .then((response) => response.json())
            .then((data) => {
                console.log("dataUpdate h gaya bhaiiii",data);
