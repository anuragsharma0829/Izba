...
// storage 
$("#PalletFee").val(extractNumericValue(parsedValues['Storage_Per_Pallet']?.value?.replace(/\$|,/g, '')));
$("#SelfFee").val(extractNumericValue(parsedValues['Storage_Per_Shelf']?.value?.replace(/\$|,/g, '')));
$("#BinFee").val(extractNumericValue(parsedValues['Storage_Per_Bin']?.value?.replace(/\$|,/g, '')));

// receving
$("#palletReciiptRate").val(extractNumericValue(parsedValues['Receipt_Per_Pallet_Fee']?.value?.replace(/\$|,/g, '')));
$("#CaseRecipRate").val(extractNumericValue(parsedValues['Receipt_Per_Case_Fee']?.value?.replace(/\$|,/g, '')));

// labour
$("#WHlabor").val(extractNumericValue(parsedValues['Labor_Warehouse_Rate']?.value?.replace(/\$|,/g, '').replace('Hourly', '')));
$("#laborItrate").val(extractNumericValue(parsedValues['Labor_IT_Rate']?.value));

// Managment 
$("#ManagmentFee").val(extractNumericValue(parsedValues['Management_Fee']?.value?.replace(/\$|,/g, '')));
$("#MinimumFee").val(extractNumericValue(parsedValues['Minimum_Fee']?.value?.replace(/\$|,/g, '')));

// parcel
$("#ParcelManifesFee").val(extractNumericValue(parsedValues['Manifest_Fee']?.value?.replace(/\$|,/g, '')));
$("#internationalFee").val(extractNumericValue(parsedValues['International_Surcharge_Fee']?.value?.replace(/\$|,/g, '')));
