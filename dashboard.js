            ...
            fileReader.onload = async function (event) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        const laborITRateValue = parsedValues['Labor_IT_Rate']?.value;
const numericValueRegex = /([\d,\.]+)/; // Regular expression to match numeric value

const matches = laborITRateValue.match(numericValueRegex);
const numericValue = matches ? parseFloat(matches[1].replace(',', '')) : 0;

$("#laborItrate").val(numericValue);
