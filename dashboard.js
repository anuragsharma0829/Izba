...
//   ******************** Cheak Brand For DashBoard Page  ************************
...
// ************************* Full fillment Contract Section ************************

// All Functions for Fullfillment contract 

// Show Detail for fulfillment Contract 
async function showDetailforContract(fcobjId){
    let contractDetails = {
        objectId: fcobjId,
    };
...
// Function for Edit Contract 

async function contractEdit(contrctId){
     // formate Date
...
async function deleteContracts(deletecID){
    Swal.fire({
        icon: "info",
        title: "Are You sure",
...
async function createServices(idforRate){
    //   Ceate Service
...
// function to edit ratecard 
async function editRateCards(editrID){
    $("#ratecardDetailSection").hide();
    $("#FulfillmentContractDetailSection").hide();
    $("#UpdateRateCard").show();
...
// Show rateCards For Partcular Contracts 


async function deletecard(objID){
    ...
    Swal.fire({
        ...
        const rateItemsPerPage = 10; // You can change the number of items per page here
        ...
        async function fetchRateData() {
            const response = await fetch('https://cleanstation.backendless.app/api/services/FulfilmentContract/IDToRateCard', {
                ...
                body: JSON.stringify({
                    "objectId": "E2602771-55C2-44D3-9D20-F087F5C20C00"
                })
            });
        ...
        async function renderRateTable(page) {
            ...
            tableBody.innerHTML = paginatedData.map(item => {
                ...
                return `
                    <tr>
                        <td onclick="handleRateData('${item.objectId}')">${item.Rate_Card_Name}</td>
    ...
    for (let i = 1; i <= totalPages; i++) {
        paginationHtml += `<li${rateCurrentPage === i ? ' class="active"' : ''}><a href="#" onclick="renderRateTable(${i})">${i}</a></li>`;
    }
    paginationHtml += `
    ...
    if (rateCurrentPage > 1) {
        rateCurrentPage--;
        renderRateTable(rateCurrentPage);
        renderRatePagination(fetchRateData().length); // Pass the totalItems argument here
    }
}

// Function to change to the next page
function nextRatePage() {
    const data = fetchRateData();
    data.then(result => {
        const totalPages = Math.ceil(result.length / rateItemsPerPage);
        if (rateCurrentPage < totalPages) {
            rateCurrentPage++;
            renderRateTable(rateCurrentPage);
            renderRatePagination(result.length);
        }
    }).catch(error => {
        ...
        function handleRateData(objectId){
            $("#ratecardDetailSection").show();
            $("#FulfillmentContractDetailSection").hide();
...
const contractItemsPerPage = 10; // You can change the number of items per page here
let currentContractId = null;
// Function to fetch data from API
async function fetchContractData() {
    const response = await fetch('https://cleanstation.backendless.app/api/services/Brand/BrandContract', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
// Function to render the table
async function renderContractTable(page) {
    const data = await fetchContractData();

    if (data.length === 0) {
        // Show the "No rate card for this contact" message if no data is available
        document.getElementById('Contract-table-body').innerHTML = '';
        document.getElementById('contract-pagination').innerHTML = '';
        document.getElementById('Contract-no-data-message').style.display = 'block';
        return;
    }

    const startIndexcontract = (page - 1) * contractItemsPerPage;
    const endIndexcontract = startIndexcontract +  contractItemsPerPage;
    const paginatedDatacontract = data.slice(startIndexcontract, endIndexcontract);

    document.getElementById('Contract-no-data-message').style.display = 'none';

    const tableBody = document.getElementById('Contract-table-body');
    tableBody.innerHTML = paginatedDatacontract.map(contract => {
        // const brandContractForContract = item.Brand_Contract ? '<span class="yes">Yes</span>' : '<span class="no">No</span>';
        return `
            <tr>
                <td onclick="handleContractData('${contract.objectId}')">${contract.Contract_name}</td>
                <td>${new Date(contract.created).toLocaleString()}</td>
               
                <td>
                    <span class="edit-btn glyphicon glyphicon-pencil" onclick="handleContractEdit('${contract.objectId}')"></span>
                    <span class="delete-btn glyphicon glyphicon-trash" onclick="handleContractDelete('${contract.objectId}')"></span>
                </td>
            </tr>
        `;
    }).join('');
...
function renderContractPagination(totalItems) {
const totalPagesforContract = Math.ceil(totalItems / contractItemsPerPage);
const paginationElementforContract = document.getElementById('contract-pagination');

let paginationHtmlCon = `
<li${contractCurrentPage === 1 ? ' class="disabled"' : ''}>
    <a href="#" aria-label="Previous" onclick="prevContractPage()">
        <span aria-hidden="true">&laquo; Previous</span>
    </a>
</li>
`;
for (let i = 1; i <= totalPagesforContract; i++) {
paginationHtmlCon += `<li${contractCurrentPage === i ? ' class="active"' : ''}><a href="#" onclick="changeRatePage(${i})">${i}</a></li>`;
}
paginationHtmlCon += `
<li${contractCurrentPage === totalPagesforContract ? ' class="disabled"' : ''}>
    <a href="#" aria-label="Next" onclick="nextContractPage()">
        <span aria-hidden="true">Next &raquo;</span>
    </a>
</li>
`;
paginationElementforContract.innerHTML = paginationHtmlCon;
}

// Function to change to the previous page
function prevContractPage() {
    if (contractCurrentPage > 1) {
        contractCurrentPage--;
        renderContractTable(contractCurrentPage);
    }
}
...
fetchContractData().then(data => {
    const totalPagesforContract = Math.ceil(data.length / contractItemsPerPage);
    if (contractCurrentPage < totalPagesforContract) {
        contractCurrentPage++;
        renderContractTable(contractCurrentPage);
    }
});
...
function changeRatePage(pageNumber) {
contractCurrentPage = pageNumber; // Set the current page to the clicked page number
renderContractTable(contractCurrentPage);
}


// Function to handle Edit button click
function handleContractEdit(objectId) {
   contractEdit(objectId)
}

// Function to handle Delete button click
function handleContractDelete(objectId) {
    deleteContracts(objectId);
}


// Function to handle Delete button click
function handleContractData(objectId) {
    // Show Particular Contract detail 
  showDetailforContract(objectId);
   //   end here

// Show Ratecards For Particular Contract

   showrateCards(objectId)
...
// Function to render the table
async function renderProductTable(page) {
    ...
    if (data.length === 0) {
        ...
        document.getElementById('product-no-data-message').style.display = 'block';
        return;
    }

    const startIndexproduct = (page - 1) * productItemsPerPage;
    const endIndexproduct = startIndexproduct + productItemsPerPage;
    const paginatedDataproduct = data.slice(startIndexproduct, endIndexproduct);

    document.getElementById('product-no-data-message').style.display = 'none';

    const tableBody = document.getElementById('product-table-body');
    tableBody.innerHTML = paginatedDataproduct.map(product => {
        // const brandContractForContract = item.Brand_Contract ? '<span class="yes">Yes</span>' : '<span class="no">No</span>';
        return `
                 <tr>
                     <td onclick="handleproductData('${product.objectId}')">${product.Name}</td>
                     <td>${new Date(product.created).toLocaleString()}</td>
                    
                     <td>
                         <span class="delete-btn glyphicon glyphicon-trash" onclick="handleproductDelete('${product.objectId}')"></span>
                     </td>
                 </tr>
             `;
    }).join('');

    renderproductPagination(data.length);
}

// Function to render pagination
function renderproductPagination(totalItems) {
    const totalPagesforproduct = Math.ceil(totalItems / productItemsPerPage);
    const paginationElementforproduct = document.getElementById('product-pagination');

    let paginationHtmlCon = `
     <li${productCurrentPage === 1 ? ' class="disabled"' : ''}>
         <a href="#" aria-label="Previous" onclick="prevproductPage()">
             <span aria-hidden="true">&laquo; Previous</span>
         </a>
     </li>
 `;
    for (let i = 1; i <= totalPagesforproduct; i++) {
        paginationHtmlCon += `<li${productCurrentPage === i ? ' class="active"' : ''}><a href="#" onclick="changeProductPage(${i})">${i}</a></li>`;
    }
    paginationHtmlCon += `
     <li${productCurrentPage === totalPagesforproduct ? ' class="disabled"' : ''}>
         <a href="#" aria-label="Next" onclick="nextproductPage()">
             <span aria-hidden="true">Next &raquo;</span>
         </a>
     </li>
 `;
    paginationElementforproduct.innerHTML = paginationHtmlCon;
}
...
function nextproductPage() {
    fetchProductData().then(data => {
        const totalPagesforproduct = Math.ceil(data.length / productItemsPerPage);
        if (productCurrentPage < totalPagesforproduct) {
            productCurrentPage++;
            renderProductTable(productCurrentPage);
        }
    });
...
function changeProductPage(pageNumber) {
    productCurrentPage = pageNumber; // Set the current page to the clicked page number
    renderProductTable(productCurrentPage);
}


// Function to handle Delete button click
function handleproductDelete(objectId) {
    deleteProduct(objectId)
}

// handle Product Cell Click
function handleproductData(objctID){
    $("#ProductSection").hide();
    $("#UploadProductFileSection").hide();
    $("#ProductDetailSection").show();
    productsAlldata(objctID);
}
