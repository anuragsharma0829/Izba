...
function handleRateCardClick(objectId) {
    ...
    function addRow(name, zoneCards) {
        ...
        // Create the collapse button and action buttons container
        ...
        // Determine the weight unit for this service
        var weightUnit = zoneCards[0]?.Weight_Unit || "LBS";

        // Sort zoneCards based on weight (either Weight_LBS or Weight_OZ)
        zoneCards.sort((a, b) => a[`Weight_${weightUnit}`] - b[`Weight_${weightUnit}`]);

        // Create the section content (table)
        ...
        table.innerHTML = `
            ...
            <tbody>
                <!-- Populate rows using zoneCards data -->
                ${zoneCards.map((zoneCard) => `
                    <tr>
                        <td>${zoneCard[`Weight_${weightUnit}`] || 0} ${weightUnit}</td>
                        <td>${zoneCard.Zone_1 || 0}</td>
                        <td>${zoneCard.Zone_2 || 0}</td>
                        <td>${zoneCard.Zone_3 || 0}</td>
                        <td>${zoneCard.Zone_4 || 0}</td>
                        <td>${zoneCard.Zone_5 || 0}</td>
                        <td>${zoneCard.Zone_6 || 0}</td>
                        <td>${zoneCard.Zone_7 || 0}</td>
                        <td>${zoneCard.Zone_8 || 0}</td>
                    </tr>
                `).join('')}
            </tbody>
        `;
    ...
    function fetchDataFromAPI() {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    // Call the function to generate sections with the response data
                    responseData.forEach(function (item) {
                        var name = item.Name;
                        var zoneCards = item.Zone_Cards || [];
                        addRow(name, zoneCards);
                    });
                } else {
        ...
        var apiUrl = "https://cleanstation.backendless.app/api/services/ZoneServices/IDToService";
        var requestBody = {
            "objectId": objectId
        };
    ...
    fetchDataFromAPI(); // Call the function to fetch data from the API and generate the sections on page load
