  ...
  function addRow(name, item) {
    ...
    if (
      ...
      item.Zone_Cards.forEach(function (zoneCard) {
        ...
        tableRow.innerHTML = `
            <td  style="background-color: green; color: white; height: 30px; text-align: center; margin-bottom: 10px;">${
              zoneCard["Weight_" + item.Weight_Unit]
            } ${item.Weight_Unit}</td>
            
            
      <td style ="padding: 2px 5px; text-align: center;"><input type="text" class="demo" data-object-id="${
        item.objectId
