...
// Function to reload the #testdrive section content
          ...
          contractsToShow.forEach((contract, index) => {
            ...
            row.innerHTML = `
                ...
                <td style="padding: 8px; border-bottom: 1px solid #ccc;">
                  <a class="contract-name" data-contract-id="${contract.objectId }">${contract.Contract_name}</a> </td>
                <td style="padding: 8px; border-bottom: 1px solid #ccc;">
  ...
  async function renderRateTable(page) {
    ...
    tableBody.innerHTML = paginatedData
      .map((item) => {
        ...
        return `
                <tr>
                    <td class="rate-card-name"> <a class="rate-card-link" href="#" onclick="handleRateCardClick('${item.objectId }')">${item.Rate_Card_Name}</a> </td>
                    <td>${item.Carrier_Type}</td>
                    <td>${brandContract}</td>
                    <td>${new Date(item.created).toLocaleString()}</td>
                    <td> <span class="edit-btn glyphicon glyphicon-pencil" onclick="handleRateEdit('${item.objectId}')"></span>
  ...
  fetch(apiUrl, {
    ...
    .then((data) => {
      ...
      $("#UpdateContractRateButton").click(function () {
        ...
        const Fulfillment_Center = localStorage.getItem("selectedEditCenterName");
...
function handleRateEdit(objectId) {
  ...
  fetch(
    ...
    .then((data) => {
      ...
      document.getElementById("updateShippingDropdown").value = localStorage.getItem("RCtype");
      document.getElementById("updatetoggleSwitch").value = localStorage.getItem("toggleValue");
      ...
      $("#updaterateCard").click(function () {
        ...
        const updatedData = {
          ...
          Brand_Contract: localStorage.getItem("updatetoggleValue"),
        };
...
function handleRateCardClick(objectId) {
  ...
  fetch(
    ...
    .then((data) => {
      document.getElementById("RateCardNameText").innerHTML =  data.Rate_Card_Name;
        const CarrierType = localStorage.getItem("RCtype");
        console.log(CarrierType);
      document.getElementById("CarrierTypeText").innerHTML = CarrierType;
  ...
  if (existingService) {
    ...
    // upIcon.style.alignItems = "center";
    // upIcon.style.cursor = "pointer";

    // var upIconButton = document.createElement("i");
    // upIconButton.className = "fa fa-chevron-up";
    // upIcon.appendChild(upIconButton);
...
//         }
//         Swal.fire("Success", "Fulfillment Center Created Successfully");
//         $("#fcForm")[0].reset();
//         $("#NewFulfillmentCenterSection").hide();
//         $("#CreateFulfillmentContractSection").show();
//       })
...
document.addEventListener("DOMContentLoaded", function () {
  let isaddFccenterBTN  = document.getElementById("addFccenterBTN");
  if(isaddFccenterBTN){
      document.getElementById("addFccenterBTN").addEventListener("click", function () {
      // Get values from the form
      var fcName = document.getElementById("fcName").value;
      var fcURL = document.getElementById("fcURL").value;
      var storageTypeDropdown = document.getElementById("storageTypeDropdown").value;
      var fcaddress = document.getElementById("fcaddress").value;
      var fcCity = document.getElementById("fcCity").value;
      var fcState = document.getElementById("fcState").value;
      var fcZipCode = document.getElementById("fcZipCode").value;
      var fcCountry = document.getElementById("fcCountry").value;

      console.log("fcName:", fcName);
      console.log("fcURL:", fcURL);
      console.log("storageTypeDropdown:", storageTypeDropdown);
      console.log("fcaddress:", fcaddress);
      console.log("fcCity:", fcCity);
      console.log("fcState:", fcState);
      console.log("fcZipCode:", fcZipCode);
      console.log("fcCountry:", fcCountry);
  ...
  // Create a JSON object with the data to send in the POST request
  var data = {
      Center_Name: fcName,
      URL: fcURL,
      State: fcState,
  ...
  .then(function (response) {
      if (response.ok) {
          ...
          document.getElementById("NewFulfillmentCenterSection").style.display = "none";
          document.getElementById("CreateFulfillmentContractSection").style.display = "block";
          document.getElementById("formMessage").style.display = "block";
          document.getElementById("formMessage").innerHTML = "Fulfillment Center Created Successfully!!";
          // Clear the form fields
          document.getElementById("fcForm").reset();
      } else {
        document.getElementById("formMessage").style.display = "block";
          document.getElementById("formMessage").innerHTML = "Error: Failed to submit the form.";
      }
  })
...
$(document).ready(function () {
  $("#addRCcardbtn").click(function () {
    ...
    let toggleSwitch = localStorage.setItem("toggleValue");
    let Carrier_Type = localStorage.setItem("RCtype");
    const rateCardmessage = document.getElementById("rateCardmessage");

    if (ratecarName !== "" || toggleSwitch !== "" || Carrier_Type !== "") {
      ...
      let ratecardDetail = {
        ...
        Brand_Contract: localStorage.getItem("toggleValue"),
      };
      fetch(
