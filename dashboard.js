...
function fetchAndShowContractDetails(contractId) {
    ...
    async function renderRateTable(page) {
        ...
        tableBody.innerHTML = paginatedData.map(item => {
            ...
            return `
                <tr>
                    <td class="rate-card-name" onclick="handleRateCardClick('${item.objectId}')">${item.Rate_Card_Name}</td>
                    <td>${item.Carrier_Type}</td>
                    <td>${brandContract}</td>
                    <td>${new Date(item.created).toLocaleString()}</td>
                    <td>
...
function handleRateEdit(objectId) {
    ...
    // Step 1: Fetch data using POST request
    fetch("https://cleanstation.backendless.app/api/services/ZoneRateCard/IDToRateCard", {
        method: "POST",
        body: JSON.stringify(ratecardID),
        headers: {
        ...
        .then((response) => response.json())
        .then((data) => {
            // Step 2: Use the fetched data to set values in the HTML elements
            document.getElementById("RateCardNameText").innerHTML = data.Rate_Card_Name;
            document.getElementById("CarrierTypeText").innerHTML = data.Carrier_Type;
            document.getElementById("BrandContractText").innerHTML = data.Brand_Contract;

            // Step 3: Save the data in a variable so that it can be used when the "Update Rate Card" button is clicked
            const updatedData = {
                "Rate_Card_ID": {
                    "objectId": objectId  // Using the objectId received in the handleRateEdit function
                },
                "Rate_Card_Name": data.Rate_Card_Name,
                "Carrier_Type": data.Carrier_Type,
                "Brand_Contract": data.Brand_Contract
            };

            // Step 4: Bind the click event for the "Update Rate Card" button
            $("#updaterateCard").click(function () {
                // Step 5: Execute the PUT request when the button is clicked
                ...
                .then((response) => response.json())
                .then((data) => {
                    // Step 6: Handle the response or perform any actions after successful update
...
function handleRateCardClick(objectId) {
    ...
    $("#CreateServiceButton").click(function () {
        ...
        if (
            ...
            serviceaddError.style.display = "hide";
            let Servicedetail = {
                Rate_Card_Id: {
                    objectId: objectId
                },
            ...
            fetch("https://cleanstation.backendless.app/api/services/ZoneServices/Create", {
                ...
                .then((data) => {
                    ...
                    serviceaddError.style.color = "white";
                    serviceaddError.style.backgroundColor = "green";
