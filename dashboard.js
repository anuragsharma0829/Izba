            ...
            fileReader.onload = async function (event) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        const extractedValue = parseFloat(parsedValues['additional pick fees']?.value?.replace(/\$|,/g, '')) || 0;

                    
                        // Update input fields with extracted values
                        $("#baseorderfee").val(parsedValues['base per order fee']?.value?.replace(/\$|,/g, '') || 0);
                        $("#additionalPickfee").val(extractedValue);
                        $("#BaseReturnFee").val(parsedValues['base per return fee']?.value?.replace(/\$|,/g, '').replace('Hourly', '')) || 0;
                        // const baseReturnFeeValue = parseFloat(parsedValues['base per return fee']
                        $("#PerKitFee").val(parsedValues['base per return fee']?.value?.replace(/\$|,/g, '').replace('Hourly', '')) || 0;

                        // storage 
                        $("#PalletFee").val(parsedValues['Storage_Per_Pallet']?.value?.replace(/\$|,/g, '') || 0);
                        $("#SelfFee").val(parsedValues['Storage_Per_Bin']?.value?.replace(/\$|,/g, '') || 0);    
                        ...
                        $("#fulfillmentContractSection").show();
                        $("#UploadSensibleFileSection").hide();
                    } else {
...
// Upload Sensible File Button
$("#UploadSensibleFileBTN").click(function () {
    $("#fulfillmentContractSection").hide();
    $("#UploadSensibleFileSection").show();
})
