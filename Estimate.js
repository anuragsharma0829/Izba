    ...
    fetch(`https://cleanstation.backendless.app/api/services/Estimate/EstimateIDToData?ID=${objectId}`).then(
        (res) => {
            res.json().then((data) => {
                ...
                document.getElementById("updateEstimateStartDate").value = data.Start_Date;
                document.getElementById("updateEstimateEndDate").value = data.End_Date;

                const timeDifference = data.Start_Date.getTime() - data.End_Date.getTime() ;
                const daysDifference = timeDifference / (1000 * 3600 * 24);
