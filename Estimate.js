...
console.log(Udataa);
if (!Udataa) {
  // "userData" is not present, or it's empty, so the user is not logged in
  // Redirect to the login page
  window.location.href = "https://izba-exchange.webflow.io/log-in"; // Replace with the actual login page URL
} else {
  const dataEstimate = JSON.parse(Udataa);

  if (!dataEstimate || !dataEstimate.token) {
    // The "userData" item doesn't contain a valid token
    // Redirect to the login page
