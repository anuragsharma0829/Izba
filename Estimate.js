    ...
    fetch(`https://cleanstation.backendless.app/api/services/Estimate/EstimateIDToData?ID=${objctId}`).then(
        (res) => {
            res.json().then((data) => {
                ...
                //  labour
                document.getElementById("ESTtotalStandardHourtext").innerHTML = data.WHHours || 0;
                document.getElementById("ESTtotalOTHourlyText").innerHTML = data.OTHours || 0;
                document.getElementById("ESTtotalITHourlyText").innerHTML = data.ITHours || 0;
                 // Price 
                ...
                document.getElementById("ESTTotalAveragePerOrder").textContent ="$" + TotalSpents;
    ...
    fetch(`https://cleanstation.backendless.app/api/services/Estimate/EstimateIDToData?ID=${objectId}`).then(
        (res) => {
            res.json().then((data) => {
                ...
                const UstartDat = new Date(data.Start_Date);
                const UendDat = new Date(data.End_Date);

                document.getElementById("updateESTNameInput").innerHTML = data.Estimate_Name;
                document.getElementById("updateCountyList").innerHTML = data.Contract_name || "Izba1";
                document.getElementById("updateRateCardList").innerHTML = EstimateRCName || "test";
                document.getElementById("updateServiceList").innerHTML = EstimateServiceName || "test";

                const startDateText = `${UstartDat.getDate()}/${UstartDat.getMonth() + 1}/${UstartDat.getFullYear()}`;
                const endDateText = `${UendDat.getDate()}/${UendDat.getMonth() + 1}/${UendDat.getFullYear()}`;

                document.getElementById("updateEstimateStartDate").textContent = startDateText;
                document.getElementById("updateEstimateEndDate").textContent = endDateText;

                const timeDifference = UstartDat.getTime() - UendDat.getTime() ;
                const daysDifference = timeDifference / (1000 * 3600 * 24);
                document.getElementById("updateEstimateTotal-Days").textContent = daysDifference + " days";


                // pick  
                document.getElementById("updateESTbaseorders").innerHTML = data.TotalOrders || 0;
                document.getElementById("updateESTbasketsize").innerHTML = data.ExtraPicks || 0;
                document.getElementById("updateESTbasereturns").innerHTML = data.Returns || 0;
                document.getElementById("updateESTkitscreated").innerHTML = data.KitsBuilt || 0;
               

                // receipet
                document.getElementById("updateESTpalletsreceived").innerHTML = data.ReceiptPallet || 0;
                document.getElementById("updateESTcasesreceived").innerHTML = data.ReceiptCase || 0;
               

                // Storage
                document.getElementById("updateESTpalletstorage").innerHTML = data.Pallets || 0;
                document.getElementById("updateESTshelfstorage").innerHTML = data.Shelfs || 0;
                document.getElementById("updateESTbinstorage").innerHTML = data.Bins || 0;
              


                //  labour
                document.getElementById("updateESTwarehousehours").innerHTML = data.WHHours || 0;
                document.getElementById("ESTOThoursinput").innerHTML = data.OTHours || 0;
                document.getElementById("ESTIThours").innerHTML = data.ITHours || 0;
