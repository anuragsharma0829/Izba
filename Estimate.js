...
document.getElementById('services').addEventListener('change', function() {
    ...
    // Retrieve Weight_Range from local storage using the selected service ID
    const weightRange = localStorage.getItem(selectedServiceId);
    if (weightRange !== null) {
        localStorage.setItem('selectedServiceWeightRange', weightRange); // Set Weight_Range in local storage
    }
});
